...
class SplitFinder:
    def __init__(self, x, g, s, lambda_v, gamma):
        ...
        self.number_of_elements = len(self.s)
    ...
    def find_best_split_feature(self, num):
        ...
        sorted_s = self.s[self.argsort_x[:, num]]
        sorted_x = self.x[self.argsort_x[:, num], num]

        Gr = sorted_g[:self.values_list[0]].sum()
        Gl = sorted_g[self.values_list[0]:].sum()

        Sr = sorted_s[:self.values_list[0]].sum()
        Sl = sorted_s[self.values_list[0]:].sum()
        for val_num in self.values_list:
            if sorted_x[val_num] == sorted_x[val_num - 1]:
                self.update_sum(Gl, Gr, Sl, Sr, sorted_g, sorted_s, val_num)
                continue

            gain = self.calculate_gain(Gl, Gr, Sl, Sr)
    ...
    def calculate_gain(self, Gl, Gr, Sl, Sr):
        gain = Gl * Gl / (Sl + self.lambda_v) + Gr * Gr / (Sr + self.lambda_v) - (Gl + Gr) * (Gl + Gr) / (
                Sl + Sr + self.lambda_v) - self.gamma
        return gain

    def update_sum(self, Gl, Gr, Sl, Sr, sorted_g, sorted_s, val_num):
        Gr += sorted_g[val_num]
        Sr += sorted_s[val_num]
...
class MyXGBoostTreeRegressor:
    ...
    def __fit_node(self, x, g, s, node_id, depth):
        if self.max_depth is not None and depth == self.max_depth:
            self.tree[node_id] = (self.LEAF_TYPE, - g.sum() / (s.sum() + self.lambda_v))
            return
        ...
        s_l = s[sorted_field][:sf.splitting_number]
        self.__fit_node(x_l, g_l, s_l, 2 * node_id + 1, depth + 1)

        x_r = x[sorted_field][sf.splitting_number:]
        g_r = g[sorted_field][sf.splitting_number:]
        s_r = s[sorted_field][sf.splitting_number:]
        self.__fit_node(x_r, g_r, s_r, 2 * node_id + 2, depth + 1)
...
if __name__ == '__main__':
    ...
    boo = XGB(n_estimators=est_num, verbose=True, learning_rate=0.1)
