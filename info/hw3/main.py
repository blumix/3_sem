...
def main():
    ...
    test_columns = ['QID'] + X_columns
    predictor = LambdaMART.LambdaMART(training_data=train.as_matrix(columns=train_colums), number_of_trees=15,
    ...
    for scores_train, scores_test in predictor.fit(test=test.as_matrix(columns=test_columns)):
        n_train = []
        for qid in set(train.QID):
            ind = train[train.QID == qid].index
        ...
        for qid in set(test.QID):
            ...
            my_sort = np.argsort(scores_test[ind])[::-1]
        ...
        if (len(ndcg_train)) % 5 == 0:
            plt.plot(range(1, len(ndcg_train) + 1), ndcg_train, label='train')
            plt.plot(range(1, len(ndcg_test) + 1), ndcg_test, label='test')
            plt.legend()
            plt.show()
        print('test', ndcg_test[-1])
...
def get_answer():
    predictor = LambdaMART.LambdaMART()
    predictor.load("temp/temp_model_50.lmart")
